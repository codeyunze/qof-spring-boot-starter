...
import io.github.codeyunze.utils.StorageStationHelper;
 ...
 * @since 2025/1/12
 */
@ConditionalOnProperty(prefix = "qof.s3", name = QofConstant.ENABLE, havingValue = QofConstant.ENABLE_VALUE)
public class S3QofClient extends AbstractQofClient implements ObjectStorageProvider {

    @Override
    public String mode() {
        return QofConstant.StorageMode.S3;
    }
    ...
    private static final Logger log = LoggerFactory.getLogger(S3QofClient.class);

    @Resource
    private S3QofProperties fileProperties;
    ...
    private S3Client getClient(QofFileOperationBase fileOperationBase) {
        ...
        String clientKey = fileStorageStation + StrUtils.toUpperCase(QofConstant.StorageMode.S3);
        S3Client client = s3ClientMap.get(clientKey);
        if (client == null) {
            throw new IllegalStateException("未找到S3客户端，存储站: " + fileStorageStation);
        }
        return client;
    }
    ...
    protected Long doUpload(InputStream fis, QofFileInfoDto<?> info) {
        ...
        try (InputStream inputStream = fis) {
            ...
            log.error("S3服务异常，文件路径: {}, 错误码: {}, 错误信息: {}", getFilePath(info), e.awsErrorDetails().errorCode(), e.getMessage(), e);
    ...
    protected QofFileDownloadBo doDownload(QofFileInfoBo<?> fileBo) {
        try {
            ...
            log.error("S3文件不存在，文件路径: {}", getFilePath(fileBo), e);
            throw new FileDownloadException("文件不存在", e);
        } catch (S3Exception e) {
            log.error("S3服务异常，文件路径: {}, 错误码: {}, 错误信息: {}", getFilePath(fileBo), e.awsErrorDetails().errorCode(), e.getMessage(), e);
