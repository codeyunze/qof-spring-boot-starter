...
|---|---|
| 文档版本 | 2.0 |
| 适用代码版本 | **17.3.2**（JDK 17 / Spring Boot 3.5.x） |
| 目标读者 | 组件维护者、接入方架构师 |
| 落地状态 | 阶段 A / B / C 已落地；17.3.2 完成 SPI 并入 core、去掉聚合 Starter、元数据 MySQL/Mongo 可插拔 |

> 本文原为 17.0.4 → 17.1/17.2 的优化方案。当前以 **17.3.2 代码为准** 描述架构；演进过程见第 10 节。  
> 变更记录见 [`CHANGELOG.md`](../CHANGELOG.md)；元数据细节见 [`元数据持久化可插拔设计.md`](元数据持久化可插拔设计.md)。

![QOF 17.3.2 架构图](qof-architecture-17.3.2.png)
...
父工程坐标：`io.github.codeyunze:qof-parent:17.3.2`。
...
├── qof-spring-boot-starter-web      # 可选 HTTP；不传递 persistence；17.3.2 起传递 local
...
Maven `<name>` 上 persistence 仍可能显示为 `qof-persistence-mysql` / `mongo`，**artifactId 以 `qof-persistence-*` 为准**。
...
- **不要同时引入** `qof-persistence-mysql` 与 `qof-persistence-mongo`（17.3.2 已去掉运行时互斥检测，后装配者会被 MissingBean 挡住，但 classpath 双依赖仍应避免）
...
17.3.2 起 `qof-spring-boot-starter-web` **传递** `qof-storage-local`。只要本地盘 + HTTP，可不再单独声明 local；COS/OSS/S3 仍需自行引入对应 `qof-storage-*`。persistence **从不**由 web 传递。
...
| PATCH（17.3.2） | 发布修正、依赖/插件、文档 |

破坏 SPI 契约将升 Major。本仓库 **不做**旧坐标别名过渡；从 17.2.x 聚合 Starter 迁到 17.3.2 需改依赖，见 [`CHANGELOG.md`](../CHANGELOG.md)。
...
| **17.3.2** | SPI 并入 core；删除聚合 Starter 与 `qof-spi`；元数据 `persistence-mysql` / `mongo`；RustFS 收敛为通用 S3 |
...
- [`docs/migration/升级指南-17.1.0.md`](migration/升级指南-17.1.0.md) / [`升级指南-17.2.0.md`](migration/升级指南-17.2.0.md)（坐标以 17.3.2 本文为准）
