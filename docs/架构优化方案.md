...
- 父工程 `qof-parent`；新增 `qof-spi` / `qof-spring-boot-autoconfigure` / 真正的 `qof-spring-boot-starter`；演示改为 `qof-examples`
...
├── qof-spi                          # 纯接口 + DTO/BO/异常/SPI（零重依赖）
...
flowchart TB
  ...
  subgraph facade["门面层 qof-spi / qof-core"]
...
2. JDK SPI：`META-INF/services/io.github.codeyunze.provider.ObjectStorageProvider`（非 Spring 场景）
...
- [ ] `S3StorageAutoConfiguration` 补进 `AutoConfiguration.imports`
...
- [ ] 抽出 `qof-spi`、`qof-spring-boot-autoconfigure`、真正的 `qof-spring-boot-starter`
